...
const prefixCls = 'el-table__header';
export default {
  name: 'ElTableHeader',
              ...
              >
              {
                this._l(columns, (column, cellIndex) =>
                  <th
                    ...
                    <div class={ ['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName] } title={column.label}>
                    {
                      column.renderHeader
		            ...
		            {
                        column.helpTipText?<div class="th-help-tip" on-mousemove={ ($event) => this.handleMouseHelpTipEnter($event, column) }  on-mouseout={ this.handleMouseHelpTipLeave }></div> : ''
                    }
                  </th>
                )
              }
  ...
  methods: {
    ...
    getHeaderCellClass(rowIndex, columnIndex, row, column) {
      ...
      //表头标注 by zhangqi
        if(column.helpTipText){
          classes.push('show-th-help-tip');
        }
    ...
    handleSortClick(event, column, givenOrder) {
       ...
       * 鼠标移入显示标注
       * */
      handleMouseHelpTipEnter($event, column){
          var $current = $($event.currentTarget);
          var sText = column.helpTipText;
          if(sText){
              var iLeft = $current.offset().left;
              var iTop = $current.offset().top;
              var sStyle = `display:block;left:${iLeft - 28}px;top:${iTop- 35}px;`;
              if($("body").find(">."+prefixCls + '-help-msg').length){
                  $("body").find(">."+prefixCls + '-help-msg').remove();
              }
              $("body").append(`<div class="${prefixCls + '-help-msg'}" style="${sStyle}">${sText}</div>`);

              //如果多行，重新定位
              var $msg = $("body").find(">."+prefixCls + '-help-msg');
              if($msg.length){
