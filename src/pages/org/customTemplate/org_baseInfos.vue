        ...
        <div class="detaile-full-main-l-item-content">
            ...
            <div v-if="!templateItem.isEdit" class="detail-edit-box">
                <Form v-for="(blockItem, blockIndex) in templateItem.data" :key="blockIndex" ref="formCustom" :model="formCustom" :rules="ruleCustom">
                    <FormItem v-for="(item, itemIndex) in blockItem.data" :key="itemIndex" v-if="item.name != 'is_phone_show'" :label="item.title" :prop="item.name" :required="item.isMust" :style="item.cellType=='textarea' ? 'width: 100%;' : ''">
                        <!-- 文件框 -->
                        <Input v-if="(item.cellType=='text' || !item.cellType) && item.name != 'cell_phone'" v-model="item.value" :maxlength="item.fieldSize" placeholder="请输入..."></Input>
                        ...
                        <!--日期处理 -->
                        <Date-picker v-if="item.cellType=='date'" v-model="item.value" placeholder="请选择" class="text-filed-length" type="date"></Date-picker>

                        <!--下拉列表 -->
                        <daydao-drop-select v-if="(item.cellType=='treeSelect' || item.cellType=='treeAdviceSelect') && ( item.name=='parent_org_id')" :isAllowOther="item.cellType=='treeAdviceSelect' ? true : false" :name="item.name" :data="item.treeSelectData" :val="item.value" :showVal="item.value"  :isEdit="item.value!=''" ref="daydaoDropSelect"></daydao-drop-select>

                        <daydao-drop-select v-if="(item.cellType=='treeSelect' || item.cellType=='treeAdviceSelect') && item.name=='person_id'" :isAllowOther="item.cellType=='treeAdviceSelect' ? true : false" :name="item.name" :data="item.treeSelectData" :val="item.value" :showVal="item.value" ref="daydaoDropSelect"></daydao-drop-select>
                    ...
                    <FormItem class="detail-edit-box-btns">
                        <Button type="ivu-btn ivu-btn-primary" @click="saveRowData('formCustom')">保存</Button>
...
export default {
    ...
    created() {
        var _this = this;
        _this.org_id = _this.$parent.$parent.id;
        _this.templateItem = JSON.parse(JSON.stringify(this.$parent.sectionItem));
        _this.editListData = _this.templateItem;
