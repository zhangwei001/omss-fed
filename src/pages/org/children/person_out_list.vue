...
<template>
    <div :class="[prefixCls,'clearfix']">
        ...
        <header :class="[prefixCls + '-header-bar']">
            ...
            <h1 :class="[prefixCls + '-header-title']">离职员工</h1>
            <span></span>
        </header>
...
const infoSetId = "person_out_list";  //页面信息集ID
...
export default {
    ...
    , data() {
        ...
        return {
            ...
            , defineBtns: [
                    ...
                    children: [
                        {
                            name: '导入离职员工', event: () => {
                                sessionStorage.setItem("importData", JSON.stringify({
                                    ...
                                    , modelId: "out_job_person_import" //导入模板的ID
                                }));
                                t.$router.push("/importData/" + infoSetId + "/single");  //单行导入
                            }
                        },
                        { name: '导出离职员工', event: () => { console.log('导出离职员工') } },
                    ],
                    event: () => {
                ...
                {
                    name: '批量再入职',
                    event: () => { this.$router.push('/batch/choose/rehire') }
                }
            ]
        };
    }
    ...
    , methods: {
        ...
        formatTableData(arr) {
            var t = this;
            return personListMethods.formatTableData(t, arr, 'outJob');
        }
        /**
         * 单元格的点击
         * */
