<template>
    <div class="detaile-full-page detail-small-page">

        <!-- title  -->
        <h3 class="detaile-full-title detail-small-title">
            <slot name="full-detail-title">
                <span>员工详情</span>
            </slot>
            <div class="detaile-small-back" @click="goToBack" style="left: 0;">
                <Icon type="chevron-left"></Icon>返回</div>
        </h3>

        <slot name="full-detail-nav">
            <!-- nav -->
            <nav class="detaile-full-nav detail-small-nav">
                <ul>
        ...
        <!-- main -->
        <div class="detaile-full-main detaile-small-main">
            <div class="detaile-full-main-l">
                <slot name="full-detail-left">
                    <div v-for="(templateItem, index) in templateDataArr" :key="index">
                        ...
                        <!-- 编辑 -->
                        <edit-list v-if="sectionItem.specialTabType == '5'" v-for="(sectionItem, index) in templateItem" :key="index" :editListData="sectionItem" :getPersonInfo="getPersonInfo"></edit-list>

                        <!-- 新增 -->
                        <add-list v-else-if="sectionItem.specialTabType == '0'" :addListData="sectionItem"></add-list>

                        <!-- 列表 -->
                        <attachment-list v-else-if="sectionItem.specialTabType == '1'" :attachmentListData="sectionItem"></attachment-list>

                        <!-- 表格 -->
                        <table-list v-else-if="sectionItem.specialTabType == '36'" :tableListData="sectionItem"></table-list>

                        <!-- 自定义模版 templateId -->
                        <custom-template v-else-if="sectionItem.specialTabType == '14'" :customTemplateData="sectionItem"></custom-template>
                    </div>
                </slot>
            </div>
        </div>
...
<script>
// 引入组件
import editList from "../detailFull/editList.vue"; //表格列表
import addList from "../detailFull/addList.vue"; //表格列表
import attachmentList from "../detailFull/attachmentList.vue"; //表格列表
import operateData from "../detailFull/operateData.vue"; //操作记录
import tableList from "../detailFull/tableList.vue"; //表格
import customTemplate from "../detailFull/customTemplate.vue"; //自定义模版
...
export default {
    ...
    created: function() {
        ...
        console.log(this.pageInfoSetId, this.id)
    },
    mounted: function() {
        this.getTableColumn();
    ...
    methods: {
         ...
         * 获取各个模版的数据
         */
        getDetailTemplateData: function() {
            var _this = this,
        ...
        navEventFn(item, index) {
            ...
            var totalH = $('.detaile-full-main-l>div').eq(index).offset().top - $('.detaile-full-title').height() - $('.detaile-full-nav').height() - $('#daydao_header_app').height();
        ...
        scrollChangeNav() {
            ...
            $(window).off('scroll').on('scroll', function() {
                ...
                $('.detail-small-title').css({
                    top: $('body').scrollTop()
                })

                $('.detail-small-nav').css({
                    top: $('body').scrollTop() + $('.detail-small-title').height()
                })
            })
        }
    }
...
.detail-small-page {
    ...
    .detaile-full-nav {
        ...
        li {
            ...
            margin-right: 12px;
            line-height: $navH; //padding: 20px 0;
            &:after {
                content: '';
                position: absolute;
                top: calc(50% - 1px);
...
.detaile-full-main-l-item-more {
    ul.detaile-full-main-l-list {
        margin: 30px 0;
        padding: 20px 0; //border: 1px solid transparent;
        background: #FCFCFC;
        box-sizing: border-box;
    }
    ...
    .detaile-full-main-l-item-content {
        ...
        &:hover {
            ...
            .detaile-full-main-l-list {
                background: #FCFCFC; //border: 1px solid #D7DDE4;
                border-radius: 2px;
            }
        }
    }
